...
import './App.css';
import left_arrow from "./assets/left-arrow-svgrepo-com.svg";
...
function App() {
  ...
  const daysInMonth=()=>{
    const daysArray=[];
    const firstDay=new Date(selectedDate.getFullYear(),
    selectedDate.getMonth(),1);
    const lastDay=new Date(selectedDate.getFullYear(),
    ...
    <>
      <div className='calendar'>
        <div className='header'>
          <button onClick={()=>{setSelectedDate(new Date(selectedDate.
          getFullYear(),selectedDate.getMonth()-1,1));}}>
            <img src={left_arrow}/>
          </button>
          ...
          <select value={selectedDate.getFullYear()} onChange={handleChangeYear}>
            {
              Array.from({length:10},(_,i)=>selectedDate.
              getFullYear()-5+i).map((year)=>(
